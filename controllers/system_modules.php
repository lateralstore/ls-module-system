	...
	protected function index_onApplyUpdates( $ignore_eula = false ) {
		try {
			if ( !$ignore_eula && Core_EulaManager::pull() ) {
				...
				Phpr::$session->flash['success'] = Phpr::$config->get( 'APP_NAME', 'Your Website' ) . ' has been successfully updated';
	...
	protected function index_onForceUpdate( $ignore_eula = false ) {
		try {
			if ( !$ignore_eula && Core_EulaManager::pull() ) {
				...
				Phpr::$session->flash['success'] = Phpr::$config->get( 'APP_NAME', 'Your site' ) . ' has been successfully updated';
				Phpr::$response->redirect( url( 'system/modules' ) );
			}
		} catch ( Exception $ex ) {
			Phpr::$response->ajaxReportException( $ex, true, true );
