<?php
	class System_Backup_Params extends Db_ActiveRecord 
	{
		...
		{
			$this->add_form_field('backup_path')->comment('Please specify an absolute path to the backup directory. The path should start from the disk root, for example: /home/user/backups.<br/><br/>Your installation directory is '.PATH_APP, 'above', true);

			$this->add_form_field('archive_uploaded_dir')->comment('Enable this option if you want to include uploaded files (website resource files - images, CSS, JavaScript files, product and category images, etc.) to backup archives. Enabling this option can significantly increase the backup file size and the server load during the archiving procedure. When this option is disabled only the database content will be archived.');

			$extraFieldClass = $this->backup_on_login ? 'separatedField' : null;
			$this->add_form_field('backup_on_login')->renderAs(frm_onoffswitcher)->comment('Enable this option if you want to create backup archives automatically when users log into the Administration Area.', 'above')->cssClassName($extraFieldClass);
